...
def pipeline_with_mocks():
    ...
    with patch("chess_analyzer.analysis_pipeline.PositionAnalyzer"):
        with patch("chess_analyzer.analysis_pipeline.PGNParser"):
            with patch("chess_analyzer.analysis_pipeline.FeatureExtractor"):
                with patch("chess_analyzer.analysis_pipeline.WeaknessClustering"):
                    pipeline = AnalysisPipeline()

                    # Setup mocks for position analyzer
                    pipeline.position_analyzer.analyze_position = Mock(
                        return_value={"evaluation": 0.5, "best_move": "e2e4", "depth": 20}
                    )
                    ...
                    pipeline.feature_extractor.extract_all_features = Mock(
                        return_value={
                    ...
                    pipeline.clusterer.label_clusters = Mock(
                        return_value={
...
def test_analyze_games_end_to_end(pipeline_with_mocks):
    ...
    # Mock PGN parser
    pipeline.pgn_parser.parse_pgn = Mock(
        return_value={
            "white": "Player1",
            "black": "Player2",
            "result": "0-1",
    ...
    assert all(
        call.kwargs["perspective"] == "white"
        for call in pipeline.position_analyzer.analyze_position.call_args_list
    )
